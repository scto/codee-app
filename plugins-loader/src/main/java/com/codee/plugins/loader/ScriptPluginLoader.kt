...
class ScriptPluginLoader(private val folder: File) : PluginLoader {
     ...
     * Loads plugin files.
     *
     * @param folder - plugin source folder. In [folder] must be `main.manifest.kts` and
     *   `main.plugin.kts` files:
     * - `main.manifest.kts` launching for first to declare info about plugin name, version and
     *   compatibility.
    ...
    override suspend fun load(
        ...
        if (!manifestFile.exists())
            return ResultWithDiagnostics.Failure(
                listOf(
                    ScriptDiagnostic(MANIFEST_FILE_DOES_NOT_EXIST, "Manifest file does not exist.")
                )
            )

        if (!pluginFile.exists())
            return ResultWithDiagnostics.Failure(
                listOf(ScriptDiagnostic(PLUGIN_FILE_DOES_NOT_EXIST, "Plugin file does not exist."))
            )
            ...
            if (!metadataResult.isError())
                ...
                ResultWithDiagnostics.Failure(
                    listOf(
                        ScriptDiagnostic(
                            NOT_LAUNCHED,
                            "Plugin wasn't launched due to error in metadata.",
                        )
                    )
                )
    ...
    private suspend fun resolveAll(dependencies: List<Dependency>): List<File> {
        ...
        return dependencies
            .mapNotNull {
                mavenResolver
                    .resolve(MavenResolverMetadata(it.repositories, it.coordinates))
                    .valueOrNull()
            }
            .flatten()
    }
