...
class DependenciesScope : IDependenciesScope {
    ...
    override fun implementation(coordinates: String, block: IDependencyScope.() -> Unit) {
...
class DependencyRepositoriesScope(private val repositories: MutableList<String>) :
    IDependencyRepositoriesScope {
    override fun maven(coordinates: String) {
        repositories.add(coordinates)
    }
}

class Dependency(val coordinates: String, val repositories: List<String>)
