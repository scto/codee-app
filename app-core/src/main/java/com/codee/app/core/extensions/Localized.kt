 ...
 * string, it searches for string with the same languageCode but without countryCode specified. Then
 * the it tries to get just any string with the languageCode
 *
    ...
    val locales =
        mutableListOf<Locale?>()
            .apply {
                ...
                for (i in 1..list.size()) add(list[i]?.toCodeeLocale())
            }
            .filterNotNull()

    for (locale in locales) {
        val languageCodeMatches = variants.filter { (k) -> k.languageCode == locale.languageCode }
        val countryCodeMatches =
            languageCodeMatches.filter { (k) -> k.countryCode == locale.countryCode }

        val result =
            countryCodeMatches.values.firstOrNull()
